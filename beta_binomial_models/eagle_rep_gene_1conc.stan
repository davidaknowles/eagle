...
model {
  ...
  for (n in 1:N) {
    for (k in 1:K) {
      ...
      for (t in 1:T) {
        log_prob_unflipped[t] <- beta_binomial_reparam_log(ys[n,t,k], ns[n,t,k], xb[t][n], conc); 
        log_prob_flipped[t] <- beta_binomial_reparam_log(ys[n,t,k], ns[n,t,k], -xb[t][n], conc);
      }
