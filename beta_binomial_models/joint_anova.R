...
joint_model_linear=stan_model("joint_model_linear.stan")
joint_model=stan_model("joint_model.stan")
gene_ase_linear=stan_model("gene_ase_linear.stan")
...
joint_linear_anova=function(ys, ns, gene_counts, library_size, xFull, xNull, concShape=1.001, concRate=0.001, algorithm="Newton",  ...) {
  ...
  dat=list(N=nrow(ys), K=ncol(ys), P=ncol(xNull[[1]]), x_1=xNull[[1]], x_2=xNull[[2]], library_size=library_size, ys=ys, ns=ns, gene_counts=gene_counts, concShape=concShape, concRate=concRate)
  ...
  init=list(beta=as.array(beta_init_null), nb_conc=100, conc=100+numeric(dat$K))
  fit_null=optimizing(model_choice, dat=dat, init=init, as_vector=F, algorithm=algorithm, ...)
  ...
  fit_full=optimizing(model_choice, dat=dat, init=init, as_vector=F, hessian=T, algorithm=algorithm,  ...)
...
joint_anova=function(ys, ns, gene_counts, library_size, xFull, xNull, concShape=1.001, concRate=0.001) {
  ...
  fit_full=optimizing(joint_model, dat=dat, as_vector=F)
  
  dat$x_1=xNull[[1]]
  dat$x_2=xNull[[2]]
  dat$P=ncol(xNull[[1]])
  
  fit_null=optimizing(joint_model, dat=dat, as_vector=F)
  
  loglr=fit_full$value - fit_null$value
  df=ncol(xFull[[1]])-ncol(xNull[[1]])
  list( loglr=loglr, df=df, lrtp=pchisq(2*loglr, df, lower.tail = F), fit_null=fit_null, fit_full=fit_full )
}