  ...
  for (n in 1:N) {
    ...
    for (t in 1:T) {
      ...
      if (library_size[n,t]>0.0) {
        real mu; 
        mu = te * library_size[n,t];
        #gene_counts[n,t] ~ normal( mu, sqrt( mu * (1.0 + mu / (conc * gene_counts[n,t])) ) ); 
        #gene_counts[n,t] ~ normal( mu, sqrt( mu * (1.0 + 1.0 / conc) ) ); 
        gene_counts[n,t] ~ normal( mu, sqrt( (sc * mu) + (1.0 / conc ) ) ); 
      }
    }
    for (k in 1:K) {
