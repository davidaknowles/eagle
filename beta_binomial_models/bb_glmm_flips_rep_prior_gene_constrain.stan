...
# One SNP version. 
# Regression coefficents are constrained positive. 
...
parameters {
  ...
  matrix[N,K] re; # random effects
  real log_rev; # log variance of random effects
}
model {
  vector[N] xb[T]; 
  real rev; 
  vector[P] beta_trans; 
  beta_trans[1] <- log1p_exp(beta[1]);
  if (P==2)
    beta_trans[2] <- log1p_exp(beta[2]) - beta_trans[1]; 
  rev <- exp(log_rev);
